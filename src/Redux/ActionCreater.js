...
export const GetCompanybycode = (code) => {
    return (dispatch) => {
        //dispatch(makeRequest());
        axios.get("http://localhost:5000/company/"+code).then(res => {
            const _obj = res.data;
...
export const CreateCompany = (data) => {
    return (dispatch) => {
        axios.post("http://localhost:5000/company", data).then(res => {
...
export const UpdateCompany = (data) => {
    return (dispatch) => {
        axios.put("http://localhost:5000/company/"+data.id, data).then(res => {
...
export const RemoveCompany = (code) => {
    return (dispatch) => {
        axios.delete("http://localhost:5000/company/"+code).then(res => {
